...
import models.baselines.hodcrnn as hodcrnn
import models.baselines.hodcrnn_tune_o as hodcrnn_tune_o
import models.baselines.hodcrnn_tune_h as hodcrnn_tune_h
...
for target_gage, upstream_gages, lags, forward, model_name, extra_label, if_tune, if_cv, test_percent, val_percent in zip(
    ...
    expr_dir = './outputs/experiments/' + model_name + extra_label + '_' + str(current_time)
    ...
    # data = data[(data.index >= '2012-09-20 00:15:00') & (data.index <= '2024-01-01 00:00:00')]
    ...
    #     (data_precip.index >= '2012-09-20 00:15:00') & (data_precip.index <= '2024-01-01 00:00:00')
    #     ]
    data_precip = None

    data_field = pp.import_data_field(f'./data/USGS_gage_field/{target_gage}.csv', to_tz='America/New_York')
    # data_field = data_field[data_field.index >= '2012-09-20']
    data_field = data_field[data_field.index >= '2017-01-01 00:00:00']
    ...
    if 'test_df' in locals():
        report_df = ev.metric_dis_pred_report(
            test_df, test_df_full, data_flood_stage['action'].iloc[0]
        )
