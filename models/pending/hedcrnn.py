...
class MultiDCRNN(torch.nn.Module):
    def __init__(self, in_channels: int, out_channels: int, K1: int, K2: int, p: float, bias: bool = True):
        ...
        self.out_channels_s = out_channels
        self.out_channels_p = out_channels
        self.K1 = K1
        self.K2 = K2
        self.bias = bias
    ...
    def _create_update_gate_parameters_and_layers(self):
        self.conv_x_z_s = DConv(
            in_channels=self.in_channels + self.out_channels_s,
            out_channels=self.out_channels_s,
            K=self.K1,
            bias=self.bias,
        )
        self.conv_x_z_p = DConv(
            in_channels=self.in_channels + self.out_channels_p,
            out_channels=self.out_channels_p,
            K=self.K2,
            bias=self.bias,
        )
        self.conv_x_z_p2s = DConv(
            ...
            # in_channels=self.out_channels_p,
            out_channels=self.out_channels_p,
            K=self.K2,
            bias=self.bias,
        )
    ...
    def _set_hidden_state(self, X_s, X_p, H_s, H_p):
        if H_s is None or H_p is None:
            H_s = torch.zeros(
                X_s.shape[0], X_s.shape[1],
                # self.out_channels_s + self.out_channels_p
                self.out_channels_s
            ).to(X_s.device)
            H_p = torch.zeros(X_p.shape[0], X_p.shape[1], self.out_channels_p).to(X_p.device)
        return H_s, H_p
    ...
    def _calculate_update_gate_enhanced(
        ...
        Z_s = torch.cat([X_s, H_s], dim=-1)
        # Z_s = torch.cat([X_s, Z_p2s, H_s], dim=-1)
        ...
        #         , Z_p
        #     ), dim=-2))
        # Z_sig_s = torch.sigmoid(
        #     torch.cat((
        #         Z_s, Z_p2s
        #     ), dim=-1))
        Z_sig_s = torch.sigmoid(Z_s)
        Z_sig_p = torch.sigmoid(Z_p)
        # Z = torch.sigmoid(
        #     Z_s
        #     + Z_p2s
        # )
        return Z_sig_s, Z_sig_p
    ...
    def _calculate_reset_gate_enhanced(
        ...
        R_s = torch.cat([X_s, H_s], dim=-1)
        # R_s = torch.cat([X_s, R_p2s, H_s], dim=-1)
        ...
        #         , R_p
        #     ), dim=-2))
        # R_sig_s = torch.sigmoid(
        #     torch.cat((
        #         R_s, R_p2s
        #     ), dim=-1))
        R_sig_s = torch.sigmoid(R_s)
        R_sig_p = torch.sigmoid(R_p)
        # R = torch.sigmoid(
        #     R_s
        #     + R_p2s
        # )
        return R_sig_s, R_sig_p
    ...
    def _calculate_candidate_state_enhanced(
        ...
        H_tilde_p = torch.cat([X_p, gated_H_p], dim=-1)
        ...
        H_tilde_s = torch.cat([X_s, gated_H_s], dim=-1)
        # H_tilde_s = torch.cat([X_s, H_tilde_p2s, gated_H_s], dim=-1)
        ...
        #         , H_tilde_p
        #     ), dim=-2))
        # H_tilde_s = torch.tanh(
        #     torch.cat((
        #         H_tilde_s
        #         , H_tilde_p2s
        #     ), dim=-1))
        H_tilde_s = torch.tanh(H_tilde_s)
        H_tilde_p = torch.tanh(H_tilde_p)
        # H_tilde = torch.tanh(
        #     H_tilde_s
        #     # + H_tilde_p2s
        # )
        return H_tilde_s, H_tilde_p

    def _calculate_hidden_state(self, Z_s, Z_p, H_s, H_p, H_tilde_s, H_tilde_p):
        # H = Z * H + ((1 - Z) * H_tilde)
        H_s = Z_s * H_s + ((1 - Z_s) * H_tilde_s)
        H_p = Z_p * H_p + ((1 - Z_p) * H_tilde_p)
        return H_s, H_p
    ...
    def forward(
        ...
        H_s: torch.FloatTensor = None,
        H_p: torch.FloatTensor = None,
    ):
        ...
        #     X_p[:, :, 0]
        # ), dim=-1), H)
        H_s, H_p = self._set_hidden_state(
            X_s[:, :, 0],
            X_p[:, :, 0],
            H_s, H_p)
        # H = self._set_hidden_state(X_s[:, :, 0], H)
        Z_s, Z_p = self._calculate_update_gate_enhanced(
            X_s, X_p,
            edge_index_s, edge_weight_s,
            edge_index_p, edge_weight_p,
            edge_index_p2s, edge_weight_p2s,
        ...
        R_s, R_p = self._calculate_reset_gate_enhanced(
            X_s, X_p,
            edge_index_s, edge_weight_s,
            edge_index_p, edge_weight_p,
            edge_index_p2s, edge_weight_p2s,
        ...
        H_tilde_s, H_tilde_p = self._calculate_candidate_state_enhanced(
            X_s, X_p,
            edge_index_s, edge_weight_s,
            edge_index_p, edge_weight_p,
            edge_index_p2s, edge_weight_p2s,
        ...
        H_s, H_p = self._calculate_hidden_state(Z_s, Z_p, H_s, H_p, H_tilde_s, H_tilde_p)
        return H_s, H_p
...
class HeteroDCRNN(nn.Module):
    def __init__(
        ...
        self.merge = DConv(
            in_channels=feat_out_dcrnn,
            out_channels=feat_out_dcrnn,
            K=K_precip,
        )
    ...
    def forward(self, x_dis, x_precip):
        ...
        # readout
        # out = self.dense_readout(h_s)
        h = torch.cat((h_s, h_p), dim=-1)
        out = self.dense_readout(h)

        if self.dense_flex_1 is not None:
            if self.tune_mode == 'w_out':
                out = out * (self.norm_param[1] - self.norm_param[0]) + self.norm_param[0]
                res = self.dense_flex_1(out)
                return out + res
            elif self.tune_mode == 'w_h':
                out = out * (self.norm_param[1] - self.norm_param[0]) + self.norm_param[0]
                res = self.dense_flex_1(h)
                res = res * (self.norm_param[1] - self.norm_param[0]) + self.norm_param[0]
                return out + res
...
def train_w_hp(
    ...
    batch_size = trial.suggest_int("batch_size", low=64, high=128, step=64)
    lr = trial.suggest_float("lr", low=0.0017, high=0.0019, step=0.0001)
    ...
    if objective_value < best_score_optuna_tune:
        ...
        torch.save(
            ...
            f'{expr_dir_global}/best_HEDCRNN_optuna_tune_{objective_value}.pth'
        )
...
def train_pred(
    ...
    if if_tune:
        study = optuna.create_study(direction="minimize",
                                    storage="sqlite:///tuner/db_dis_pred_hedcrnn.sqlite3",
                                    ...
                                    study_name="dis_pred_hedcrnn_" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        study.optimize(
